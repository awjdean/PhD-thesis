...
This means that a transformation $d \in D$ is a sequence of atomic transformations
\begin{equation}
...
We also can extend the atomic transformation composition operator $\hat{\circ}$ to the transformation operator on the entire set $D$ as:
\begin{equation}
\begin{aligned}
	& \circ: D \times D \to D \\
...
\end{equation}
$\circ$ is associative due to $\hat{\circ}$ being associative.



\footnote{This operator $\hat{\circ}$ is the operator of concatenation of walks of length 1.}
\footnote{$\circ$ is the operator of concatenation of walks.}
...
We call the associated transformation in $D$ of this trivial walk the \emph{trivial transformation at $w$} and denote it by $1_{w}$\footnote{Trivial transformations essentially represent no change happening in the world; we will hear more from trivial transformations when we look at the actions of agents.}
.
...
\draftnote{blue}{Include as footnote}{
Comment about how in the continuous case we could consider atomic transformations to be infinitesimal transformations.
}
\draftnote{red}{DIVIDER}{}



(\textit{i.e.} the \emph{dynamics of the world}).
...
\draftnote{blue}{To do}{
Insert bit about the sequence function $\operatorname{Seq}$.
"
