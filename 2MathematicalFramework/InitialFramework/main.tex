...
\textbf{Next up:}
\begin{enumerate}
    \item Edit equivalence relation definition to fit $\bot$.
    \item 
\end{enumerate}
}
...
---
\paragraph{5. Equivalence relation.}
...
(1) Is the operator $\circ_{\sim}$ well-defined ?
...
(3) We define an equivalence relation $\sim$ such that $a \sim a'$ iff $a \ast w = a' \ast w$ for all $w \in W$.
...
\begin{definition}[Equivalence of actions under $\sim$]
    ...
    \draftnote{blue}{awjdean}{Better way to write this ?}
\end{definition}
...
\textbf{Up next:}
\begin{enumerate}
    \item Associative property of $\hat{A}^{\ast}/\sim$.
    \item Potential identity property of $\hat{A}^{\ast}/\sim$.
    

    \item Example.
    \begin{enumerate}
        \item \sout{Map showing actions being sent to their equivalence classes under $\sim$: $D_{A}$ -($l$)-> $\hat{A}^{\ast}$ -($\pi_{A}$)-> $\hat{A}^{\ast}/\sim$.}
        \item \sout{World diagram with equivalence classes under $\sim$.}
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}[(1)]
    \item $(\hat{A}^{\ast}, \circ, \ast)$ --> partial action monoid.
    \item $(\hat{A}^{\ast}/\sim, \circ, \ast)$ --> partial transformation monoid.
    
\end{enumerate}

\draftnote{blue}{awjdean}{Two algebras: $(\hat{A}^{\ast}, \circ)$ and $(\hat{A}^{\ast}, \circ, \ast)$}

\textbf{Up next:}
\begin{enumerate}
    \item \sout{Define (basic, global) equivalence relation.}
    \item \sout{Prove $\sim$ is an equivalence relation.}
    \item \sout{Define canonical projection map.}
    \item \sout{$\circ_{\sim}$, $\ast_{\sim}$}
    ...
    \item Example.
    \begin{enumerate}
        \item Map showing actions being sent to their equivalence classes under $\sim$: $D_{A}$ -($l$)-> $\hat{A}^{\ast}$ -($\pi_{A}$)-> $\hat{A}^{\ast}/\sim$.
        \item World diagram with equivalence classes under $\sim$.
    \end{enumerate}
\end{enumerate}
