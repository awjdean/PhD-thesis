...
\begin{figure}[H]
    ...
    A representative diagram of an agent as a MArkov blanket.
        ...
        \draftnote{blue}{Agent as a Markov blanket image reference}{
...
The set $\hat{A}$ of atomic actions is our mathematical interpretation of the agent's effector states, and the observation process $b$ is our mathematical interpretation of the agent's observation states.\draftnote{blue}{Work on this}{}
...
\begin{enumerate}
	\item information is not lost during the observation process \draftnote{blue}{Consider}{
    Info could be modified by a constant amount, which would just shift the entire representation by a constant amount and therefore the representation would be unchanged - implies some sort of gauge invariance ?
    Also, add a footnote about how we only need that "necessary information is not lost"
    };
...
Unless stated otherwise, we will only consider agents with ideal sensors.

\draftnote{blue}{Consider}{
I think we might be able to prove something about it the agent's sensors are ideal (or effectively ideal), then the most information efficient (universal?) form of the agent's representation $\mathscr{Z}$ will make $f$ bijective ?
Does learning follow the principle of most efficient representation ?
}
...
\begin{propositionE}[][normal]
    For agents with ideal sensors \draftnote{blue}{consider}{and a suitable learning process}, the map $f$ is bijective.
    \draftnote{blue}{Consider}{Change this to" "for an agent with non-ideal sensors, $f$ cannot be}
\end{propositionE}
\begin{proofE}
...
\draftnote{purple}{PS}{Improve this intro.}
...
\newthought{It is clear,} that the construction of $\mathscr{W}_{\mathscr{A}}$ requires both the world $\mathscr{W}$ and the agent $\mathscr{A}$; therefore it makes sense to consider world-agent pairs $\mathscr{W}$-$\mathscr{A}$ instead of worlds and agents individually
\draftnote{blue}{To do}{\footnote{
...
Define \emph{perfect learning process} in the "Introducing an agent" section.
}
We also assume the learning process is \emph{perfect}; at the end of a perfect learning process, the agent's representation $\mathscr{Z}$ is isomorphic to the structure of the world $\mathscr{W}_{\mathscr{A}}$:
\begin{equation}
    \mathscr{Z} \cong \mathscr{W}_{\mathscr{A}}.
\end{equation}
...
\newthought{There are 24} atomic transformations in $\mathscr{W}_{\alpha}$ (given in \cref{tab:2x2_gridworld_length_1_transformations})\footnote{
    ...
    \draftnote{blue}{(see proposition ???)}{}; 
...
Looking at the world diagram of the atomic multigraph $\hat{\mathscr{W}}_{\alpha}$ of $\mathscr{W}_{\alpha}$ can give us a more intuitive sense of the structure of $\mathscr{W}_{\alpha}$ than looking at the lists of $D_{\varepsilon}$ and $\hat{D}$ in \cref{eqn:list_of_D_epsilon} and \cref{tab:2x2_gridworld_length_1_transformations}:\footnote{
    ...
    For example:\draftnote{blue}{
    Change this to be $\hat{\mathscr{W}}_{\alpha}$.
    }{}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \node (left) {\includegraphics[width=0.6\textwidth]{2MathematicalFramework/Images/2x2_cyclical_minimum_transformations.png}};
        \node (sim) [right=1cm of left] {\Huge$\xrightarrow{\hat{l}}$};
        \node (right) [right=1cm of sim] {\includegraphics[width=0.6\textwidth]{2MathematicalFramework/Images/2x2-cyclical-min-actions.drawio.png}};
    \end{tikzpicture}
    \caption{
