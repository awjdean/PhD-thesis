...
\begin{enumerate}
    \item \textbf{Presence of $\varepsilon$ in the agent's algebra.}
    We need to be a little careful with $\varepsilon$ because, even though $\varepsilon$ is always in $\hat{A}^{*}$ for mathematical reasons, if none of the agent's actions are equivalent to $\varepsilon$, then we might not want to include it in our algebra of the transformations of the actions of the agent.
...
We will predominantly focus on what we want the structure of $\mathscr{Z}$ to be at the end of the learning process without considering what happens to $\mathscr{Z}$ during the learning process \draftnote{blue}{awjdean}{Explain why.}.
...
Does $\mathscr{Z}$ need to have the requirement that the $\hat{D}_{Z}$'s combine associatively to give the $D_{Z}$ ?
...
% Define column types for better alignment
\newcolumntype{P}[1]{>{\raggedright\arraybackslash}p{#1}}
\newcolumntype{Q}{>{\raggedright\arraybackslash}p{0.44\textwidth}}


\begin{longtable}{@{}P{0.24\textwidth} P{0.26\textwidth} Q@{}}
\toprule
...
\bottomrule
\caption{A correspondence table showing how the key concepts of symmetry-based disentangled representation learning (SBDRL) align with objects from our framework.}
...
\footnote{
\draftnote{blue}{Consider}{Move or adjust the below now these definitions use $a$ instead of $[a]_{\sim}$.}
...
\begin{propositionE}\label{prp:global_inverse_means_inverse_in_global_algebra}
\begin{enumerate}
...
\begin{propositionE}\label{prp:undefined_action_not_globally_reversible}
...
\draftnote{blue}{proofE}{if $(\hat{A}^{*}/\sim, \circ_{\sim})$ is a group, then $\ast$ is a group action.
