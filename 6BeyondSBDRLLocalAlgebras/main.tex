...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{
Worlds with uniform algebraic structures
}
...
\begin{definition}[Action-homogeneous world]\label{def:action_homogeneous}
    A world $\mathscr{W}$ is called \emph{action-homogeneous} from a world state $w \in W$ if
    \begin{equation}
        \mathscr{W}^{\hat{A}\to}(w) \cong \mathscr{W}^{\hat{A}\to}(w') \quad \text{for all $w' \in W^{\hat{A}\to}(w)$}.
    \end{equation}
\end{definition}

\draftnote{blue}{To do}{
Check this is how the isomorphism is defined in proofs make consistent.
}
