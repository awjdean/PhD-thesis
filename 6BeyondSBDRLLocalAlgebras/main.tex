...
\begin{enumerate}
    ...
    We can write $b$ as $b = \hat{a} \circ b'$ where $\hat{a} \in \hat{A}/\sim_{w}$ and $b' \in \hat{A}^{*}$.
...
\begin{propositionE}
    ...
    \draftnote{blue}{Consider}{
    Isn't $a \ast w = a' \ast w$ for all $w \in W$ enough for the property since $\hat{A}^{*}$ contains all strings of $\hat{A}$.
    }
\end{propositionE}
\begin{proofE}
